...
class CMAES:
  ...
  def __init__(self, num_params,      # number of model parameters
    ...
    import cma
    self.es = cma.CMAEvolutionStrategy( self.num_params * [initial_weight],
                                        self.sigma_init,
  ...
  def tell(self, reward_table_result):
    reward_table = -np.array(reward_table_result[0])
...
class SimpleGA:
  ...
  def tell(self, reward_table_result):
    ...
    assert(len(reward_table_result[0]) == self.popsize), "Inconsistent reward_table size reported."
...
class MAP_Elites:
  ...
  def tell(self, reward_table_result):
    ...
    reward_table = np.array(reward_table_result[0])
    behavior_table = np.array(reward_table_result[1])

    if self.weight_decay > 0:
      l2_decay = compute_weight_decay(self.weight_decay, self.solutions)
      reward_table += l2_decay

    for idx, behavior in enumerate(behavior_table):
      assigned_bin = (behavior[0]//2, behavior[1]//2)
      if assigned_bin not in self.archive.keys():
        self.archive[assigned_bin] = (self.solutions[idx], reward_table[idx])
      elif reward_table[idx] > self.archive[assigned_bin][1]:
        self.archive[assigned_bin] = (self.solutions[idx], reward_table[idx])
    
    self.curr_best_reward = np.max(reward_table)

    if self.first_iteration or (self.curr_best_reward > self.best_reward):
      self.first_iteration = False
      self.best_reward = self.curr_best_reward
      pos = np.argmax(reward_table)
      self.best_param = np.copy(self.solutions[pos])
...
class OpenES:
  ...
  def tell(self, reward_table_result):
    ...
    reward = np.array(reward_table_result[0])
...
class DE:
  ...
  def ask(self):
    ...
    elite_range = range(self.elite_popsize)
    for i in range(self.popsize):
      idx_a, idx_b, idx_c = np.random.choice(elite_range, size=3, replace=False)
      a = self.solutions[idx_a]
      b = self.solutions[idx_b]
      child_params = mate(self.elite_params[idx_a], self.elite_params[idx_b])
      candidates.append(child_params + self.epsilon[i])
